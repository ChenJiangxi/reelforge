...
配音现在是整条片一次合成、不再逐拍变语气(接口限制,不是没实现),所以 say.speed/pitch/emotion
只是帮你想清楚"这拍该是什么感觉"的辅助字段,不直接决定合成参数——真正决定节奏起伏的是
gap_after(0.05-0.6,转折/反转给到 0.3+,钩子给 0.08-0.15)和 tts 里的停顿标记。
停顿标记只写在 tts 里,text 保持干净。visual_type:关系/相互作用(生克合冲)用 diagram,多方对照用 table,流程步骤用 flow,金句用 quote,关键数字用 data;纯文字 text 只是兜底——知识内容必须有结构。`,
    },
  ],
...
6. 念法:逐拍看 gap_after 和 tts 里的停顿。全片是不是一处停顿都没有(那就是念经)?
   关键数字和反常识的转折前面有没有停顿?转折/落点的 gap_after 够不够长(留出"沉下来"的空间)?
